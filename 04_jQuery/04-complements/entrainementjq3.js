$(function(){
    ...
    $('#galeries h3').click(function(){
        ...
        $(this).next().children('img').fadeOut(1000); // la méthode children() permet de sélectionner TOUS les éléments enfants directs d'une balise, ici toutes les balises <img> qui se trouvent dans <div.galerie> qui suit directement le <h3> que l'on a cliqué. Sans argument, children() sélection les enfants directs sans distinction.
    });
    ...
    $('#galeries img').click(function(){
        $(this).parent('.galerie').css({ border : '2px solid red' }); // la méthode parent() permet de sélectionner l'élément parent direct, ici le <div.galerie> dont on met la bordure en rouge. Sans argument parent() sélection le parent direct sans distinction.
    });


    // La méthode prev() (pour previous = précédent), le contraire de next() :
    //
    $('#galerie p').click(function(){
        $(this).prev().css({ border : '' }); // prev() permet de sélectionner l'élément directement précédent, ici le <div.galerie> précédent dont on rréinitialise la bordure. prev() peut prendre un sélecteur en argument pour ne sélectionner que les balises précédentes répondantes à celui-ci.
    });
