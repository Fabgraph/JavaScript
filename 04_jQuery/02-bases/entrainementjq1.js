...
$(function(){ // ce code s'exécutera une fois le document HTML totalement chargé
    ...
    //---------
    // Fonction principale de jQuery, alias $, sélecteur et évènement :
    ...
    $('#carre').click(function(){  // sélectionne la balise #carre et au clic => sélection toutes les balises h3 et les efface (sans écrire de boucle)
        $('h3').hide(); 
    });
    ...
    $('#monBouton').off('click'); // en ajoutant cette ligne, on dissocie les clicks de l'élément #monBouton : les clics précédemment liés au bouton ne fonctionnent plus ! C'est un moyen simple de supprimer un évènement.
    ...
    // La méthode css() permet de modifier des rpopriétés CSS écrites dans un objet : on les met entre des {} et sont séparées par une ",". Notez que les propriétés CSS qui contiennent un tiret s'écrivent dans des quotes.
    ...
    //-----------
    // Le mot clé this :
    // this est nécessaire lorsque l'on sélection plusieurs éléments simultanement et qu'on a besoin d'en cibler un en particulier :
    ...
    console.log('La propriété position du div est : ' + position);  // la méthode css() avec seulement une proppriété sous forme de string en argument est un getter : elle fournit la valeur de la propriété spécifiée, même si celle-ci n'est pas dans un attribut style (va chercher y compris dans les feuilles de style)


    //------------------
    // Accéder aux attributs des balises avec attr() :
    if ( $('.modifAlt').attr('alt') == '' ) {  // attr() avec un seul argument est un getter : il permet de récuper la valeur de l'attribut précisé (ici de alt)
    ...
    $('#afficher').click(function(){
        ...
        $('#galerie img').show(); // puis on affiche toutes les images (pour voir lerésultat)
    });
    ...
    $('#afficher-premier').click(function(){
        $('#galerie img:first').show();  // sélectionne la première image et l'affiche. Marche aussi avec :last pour la dernière. Si vous voulez cibler une imge au milieu, utilisez la méthode eq().
